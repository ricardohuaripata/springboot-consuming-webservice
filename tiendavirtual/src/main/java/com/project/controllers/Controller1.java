...
import com.project.api.TiendaVirtualApi;
...
public class Controller1 {
	...
	public ModelAndView index(HttpSession session, HttpServletRequest request) {
		...
		try {
			listaProductos = TiendaVirtualApi.obtenerProductos();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public ModelAndView listado_ventas(HttpSession session, HttpServletRequest request) {
		...
		try {
			pedidos = TiendaVirtualApi.obtenerPedidos();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public ModelAndView ver_perfil(HttpSession session, HttpServletRequest request) {
		...
		ArrayList<Pedido> userOrders = new ArrayList<Pedido>();
		
		try {
			userOrders = TiendaVirtualApi.obtenerPedidosUsuario(idUsuario);
		} catch (IOException e) {
			e.printStackTrace();
		}
